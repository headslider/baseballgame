...
function renderQuizMasterRanking(box,data,mode="result"){
  ...
  const rankingHtml=topRows.length?`<ol>${topRows.map(r=>{const score=Number(r.total_score||r.score||0);const title=r.title_info||quizMasterTitleForScore(score,data&&data.titles);return `<li class="${r.player_id===STATE.playerId?"me":""}"><span>${escapeHtml(r.rank)}位</span>${quizMasterLevelIconHtml(title.level,'qm-icon-rank')}<div class="rank-info"><div class="rank-main"><b>${escapeHtml(r.player_id)}</b><em>${escapeHtml(score)} pt</em></div><div class="rank-sub"><small>プレイ数 ${escapeHtml(r.plays||0)}回</small><small>完全制覇 ${escapeHtml(r.cleared_count||0)}回</small></div></div></li>`}).join("")}</ol>`:'<p>まだランキングはありません。</p>';
