...
- ゲームデータ読込改善も同文書に記載済み。通常ゲーム、野球博士、ランキング、お知らせは timeout とロードトークンで古い読込結果を破棄する。データ読込完了前にタイマー開始、ライフ消費、警告付き中断を発生させない。
- 野球博士チャレンジの椅子は背景レイヤー固定が正しい仕様。HTMLの `.quiz-master-chair` を動かして表示する仕様へ戻さない。
